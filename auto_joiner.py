...
def wait_until_found(sel, timeout, print_error=True):
    try:
        element_present = lambda x: x.find_element_by_css_selector(sel).is_displayed()
...
def join_meeting(meeting):
    ...
    print("[debug] Looking for joining button")
    join_now_btn = wait_until_found("button[data-tid='prejoin-join-button']", 30)
    #join_now_btn = browser.find_element_by_xpath('//*[@id="prejoin-join-button"]')
    ...
    if 'random_delay' in config:
        time_offset = meeting.time_started - get_time_in_seconds()
        ...
        if time_offset < 0:
            print("The meeting has already started, joining without delay...")
            delay = 0
        else:
            delay += time_offset
...
def get_meeting_members():
    ...
    # meeting_elems = browser.find_elements_by_css_selector("roster-button")
...
def hangup():
    ...
    try:
        switch_to_teams_tab()
        hangup_btn = browser.find_element_by_id("hangup-button")
        hangup_btn.click()
...
def handle_leave_threshold(current_meeting_members, total_meeting_members):
    ...
    if 0 < current_meeting_members <= 2:
